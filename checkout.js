const products = [
  ...
  { name: 'Custom Graphic Hoodie', price: 198, detail: 'Black / your 512 × 512 artwork', image: null }
];
const cart = JSON.parse(localStorage.getItem('aureliaCart') || '[]');
const items = document.querySelector('#summary-items');
let subtotal = 0;
if (!cart.length) items.innerHTML = '<p class="summary-note">Your bag is currently empty. Return to the collection to choose a piece.</p>';
else cart.forEach((item) => {
  const product = products.find((entry) => entry.name === item.name) || products[0];
  subtotal += product.price;
  const customImage = item.custom ? localStorage.getItem('aureliaCustomDesign') : null;
  const image = customImage || product.image;
  items.insertAdjacentHTML('beforeend', `<div class="summary-item">${image ? `<img src="${image}" alt="${product.name}">` : ''}<div><strong>${product.name}</strong><small>${product.detail}</small></div><b>$${product.price}</b></div>`);
});
...
document.querySelector('#checkout-form').addEventListener('submit', (event) => { event.preventDefault(); if (!cart.length) { window.location.href = 'index.html#collection'; return; } document.querySelector('#order-number').textContent = `AA-${Math.floor(100000 + Math.random() * 900000)}`; document.querySelector('#confirmation').classList.add('visible'); localStorage.removeItem('aureliaCart'); localStorage.removeItem('aureliaCustomDesign'); });
